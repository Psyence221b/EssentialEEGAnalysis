function varargout = process_conn_DTF( varargin )
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Directed Transfer Function (DTF)';
    sProcess.FileTag     = 'DTF';
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    DTFresult = cell(Num_scout,Num_scout);
    for row=1:Num_scout
    for col=1:Num_scout
      DTFresult{row,col} = squeeze(granger.dtfspctrm(row,col,:))';
    end
    end    
    
    % plotting & save
    fprintf('\n Preparing for plotting DTF result..... \n');
    cwd = pwd;
    ...
    save(outfile, 'DTFresult');

    cd(cwd); % back to working folder
    
    ResultMat = db_template('timefreqmat');
    ResultMat.Comment      = sprintf(['DTF(%.0fms-%.0fms)'],sProcess.options.timewindow.Value{1}(1)*1000, sProcess.options.timewindow.Value{1}(2)*1000);
