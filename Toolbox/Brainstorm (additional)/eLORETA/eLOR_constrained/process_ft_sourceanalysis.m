...
%%% DATA OR NOISE COVARIANCE ????
        ...
        for iInput = 1:length(iChanInputs)
            ...
            %----------------------- add by SIU
            % this line for dipole fixation to 1 dipole direction
            % if let 'unconstrained(3 direction) dipole, it is impossible
            % to calculate some connectivities (e.g., PLV)
            if strcmp(Method, 'eloreta')
                clear NumDipoles Alpha tmp Wq WQ TMP_HeadModelMat i
                NumDipoles = size(HeadModelMat.GridLoc,1);
                Alpha = ones(1,NumDipoles);
                for i = 1:NumDipoles
                ...
                TMP_HeadModelMat = HeadModelMat;
                TMP_HeadModelMat.Gain = TMP_HeadModelMat.Gain * WQ;
            end
            %----------------------- add by SIU
            
            % Convert head model to FieldTrip format
            [ftHeadmodel, ftLeadfield] = out_fieldtrip_headmodel(TMP_HeadModelMat, ChannelMat, iChannelsData, 1);
            ...
            switch (Method)
                ...
                %----------------------- add by SIU
                case 'eloreta'
                    DataCovFile = sStudyChan.NoiseCov(2).FileName;
                    DataCovMat = load(file_fullpath(DataCovFile));
                    ftData.cov = DataCovMat.NoiseCov(iChannelsData,iChannelsData);
                    
                    cfg.channels = {'all','-UVEO','-LVEO','-RHEO','-LHEO'};
                    cfg.covariance = 'yes';
                    cfg.keepmom = 'yes';
                    cfg.keepfilter = 'yes';
                    cfg.normalize = 'yes';
            ...
            %----------------------- add by SIU
            if strcmp(Method, 'eloreta')
                fprintf('\nCalculation neural time course in brain... \n');
                clear A L
                
                L = zeros(size(ftHeadmodel.label,1),NumDipoles);
                A(1:NumDipoles) = deal(struct('Ua',[],'Sa',[],'Va',[]));
                                
                ftSource.avg.pow = [];
                for ind_dipole = 1:NumDipoles
                    % Replace avg.pow by moment neural index in Fieldtrip
                    ftSource.avg.pow(ind_dipole ,:) = ftSource.avg.mom{ind_dipole}./3;
                    % Reshape leadfield matrix to brainstorm format
                    L(:,ind_dipole) = ftSource.leadfield{ind_dipole};
                end
                
               
                for i = 1:NumDipoles
                   ndx = ((1-1):0) + i*1; % ndx = ((1-NumDipoleComponents):0) + i*NumDipoleComponents;
                   [Ua,Sa,Va] = svd(L(:,ndx),'econ');
                   Sad = diag(Sa);
                   
                   tol = length(Sad) * eps(single(Sad(1)));
            ...
            %----------------------- add by SIU
            if strcmp(Method, 'eloreta')
                ResultsMat.ImageGridAmp  = ftSource.avg.pow;
                ResultsMat.SourceDecompVa = [A.Va];
                ResultsMat.SourceDecompSa = [A.Sa];
            else
