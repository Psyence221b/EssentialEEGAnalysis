...
%                 cfg2 = [];
%                 cfg2.projectmom = 'yes';
%                 ftSource = ft_sourcedescriptives(cfg2, ftSource);
%                 clear cfg2
                NumDipoles = size(ftSource.avg.mom,2);
                L = zeros(size(ftHeadmodel.label,1),NumDipoles*3);
                clear A
                ...
                for ind_dipole = 1:NumDipoles
                    ...
                    ftSource.avg.pow((ind_dipole-1)*3+1:(ind_dipole-1)*3+3 ,:) = ftSource.avg.mom{ind_dipole};
                    % Reshape leadfield matrix to brainstorm format
                    L(:,(ind_dipole-1)*3+1:(ind_dipole-1)*3+3) = ftSource.leadfield{ind_dipole};
                end
