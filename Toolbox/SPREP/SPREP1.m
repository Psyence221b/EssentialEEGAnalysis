...
%% parameter
LP_filterHz = 114; %(100 cut-off)
HP_filterHz = 1; %(0.5 cut-off)
...
%% Low-pass filter
EEG = pop_eegfiltnew(EEG, 0, LP_filterHz);
fprintf(logFile, ['%.2f - Low-passed 1Hz(0.5 cut-off)  \n'], toc);

%% High-pass filter
EEG = pop_eegfiltnew(EEG, HP_filterHz, 0);
EEG.setname = [name '_0.5Hz'];
fprintf(logFile, ['%.2f - High-passed 1Hz(0.5 cut-off)  \n'], toc);
